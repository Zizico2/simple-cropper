  ...
  [],
  {
    variants: {
      type: {
        ...
        overlay: "text-base absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2",
      },
      visibleWhen: {
...
export default function ImageUpload({ onImageSelected }: ImageUploadProps) {
  ...
  return (
    ...
    >
      ...
      <p
        data-drag-active={isDragActive}
        className={dropzoneTextStyles({ type: "secondary", visibleWhen: "idle" })}
      >
        Supports JPG, PNG, WEBP, BMP
      </p>
      <p
        data-drag-active={isDragActive}
        className={dropzoneTextStyles({ type: "overlay", visibleWhen: "active" })}
      >
        Drop the image here ...
      </p>
    </div>
